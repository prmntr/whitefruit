#!/usr/bin/env python3
"""whitefruit: download YouTube Music playlists, keep them deduped, and sync
them into iTunes as playlists.

Run with no arguments for an interactive menu. Put playlist URLs in
playlists.txt first.

Non-interactive subcommands are also available for scripting:
    download        fetch/update playlists listed in playlists.txt
...
from whitefruit import download, dedupe as dedupe_mod, itunes, repair as repair_mod, settings
...
def value_style(key: str, val, cfg: dict, checked=None):
    ...
    if key == "cookies_from_browser":
        ...
        return C.YELLOW, f"{C.YELLOW} — not signed in, Premium tracks fall back to search{C.RESET}"
    if key == "cookies_file":
        path = (val or "").strip()
        if not path:
            return C.DIM, ""
...
def splash():
    ...
    title = "whitefruit"
    tagline = "A YouTube Music to iTunes/iPod converter."
...
def run_menu():
    ...
    while True:
        ...
        print(
            ...
            f"     {C.DIM}Download new tracks, dedupe, then sync to iTunes.{C.RESET}\n"
            f"{C.CYAN}2){C.RESET} Download / update local playlists\n"
            f"     {C.DIM}Fetch anything in playlists.txt that's not on disk yet.{C.RESET}\n"
            ...
            f"{C.CYAN}6){C.RESET} Settings\n"
            f"     {C.DIM}format {cfg['audio_format']} · {cfg['audio_bitrate']} · {cfg['music_dir']}{C.RESET}\n"
            f"{C.CYAN}7){C.RESET} Quit\n"
        )
        choice = input(f"{C.BOLD}Choice [1-7]: {C.RESET}").strip()
        if choice == "7":
            return
        if choice not in ("1", "2", "3", "4", "5", "6"):
            print(f"{C.YELLOW}Please enter a number from 1 to 7.{C.RESET}")
            pause()
            continue

        clear_screen()
        if choice == "1":
            print(f"{C.YELLOW}{C.BOLD}[!] Auto mode runs start to finish without asking anything.{C.RESET}")
            ...
            print(f"{C.YELLOW}    Check option 6 first if that isn't what you want.{C.RESET}\n")
            ...
            if ok and not skipped:
                print(f"\n{C.GREEN}{C.BOLD}✓ All steps complete, no errors{C.RESET}")
            elif ok and skipped:
